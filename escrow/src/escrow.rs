...
impl<M:ManagedTypeApi> Offer<M>{
    pub fn new(creator: ManagedAddress<M>,
        offered_payment: EsdtTokenPayment<M>,
        accepted_payment: EsdtTokenPayment<M>,
        accepted_address: ManagedAddress<M>) -> Self {
            Offer {
                creator,
                offered_payment,
                accepted_payment,
...
pub trait Escrow {
    ...
    fn create_offer(&self,
        ...
        -> OfferId {
            ...
            self.created_offers(caller.clone()).insert(new_offer_id);
            self.wanted_offers(accepted_address.clone()).insert(new_offer_id);
    ...
    fn cancel_offer(&self, offer_id: OfferId) {
        ...
        self.wanted_offers(offer.accepted_address).swap_remove(&offer_id);
    ...
    fn accept_offer(&self, offer_id: OfferId) {
        ...
        require!(offer.accepted_address == caller, "Only the accepted address can accept the offer");
        require!(offer.accepted_payment == payment, "Incorrect payment for the offer");

        self.created_offers(caller).swap_remove(&offer_id);
        self.wanted_offers(offer.accepted_address.clone()).swap_remove(&offer_id);


        // Clear the offer
        self.offers(offer_id).clear();

        // Transfer offered payment to the accepted address
        self.send().direct_esdt(
        ...
        // Transfer accepted payment to the creator
        self.send().direct_esdt(
            &offer.creator,
    ...
    fn get_wanted_offers(&self, address: ManagedAddress)
    -> MultiValueEncoded<MultiValue2<OfferId, Offer<Self::Api>>> {
        let mut result = MultiValueEncoded::new();
